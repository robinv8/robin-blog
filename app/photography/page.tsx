...
export default async function Photography() {
  const posts = (await getAllPosts({ onlyPhotography: true } as any)) || [];
      ...
      <main className="columns-1 sm:columns-2 md:columns-3 gap-6 space-y-6 pb-12">
        ...
        {posts.map((post: Post, i: number) => (
          <Reveal key={post.id} delay={(i % 3) * 80} className="break-inside-avoid">
            <div className="group cursor-zoom-in border border-[#1B1B18]/20 dark:border-[#E8E6DF]/20 p-1.5 hover:border-[#FF4D00] transition-colors">
